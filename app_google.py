...
def extract_all_files(row):
    files = []
    main_file = str(row.get('file_link', '')).strip()
    if main_file and main_file.startswith('http'): files.append(main_file)
    log_text = str(row.get('logs', ''))
    log_links = re.findall(r'(https?://[^\s]+)', log_text)
    for link in log_links:
        if link not in files: files.append(link)
    return files

# --- 3. KẾT NỐI GOOGLE (CÓ CACHE ĐỂ CHỐNG LỖI 429) ---
@st.cache_resource
def get_gcp_creds():
...
# --- 4. LOGIC HỆ THỐNG (CACHE DỮ LIỆU) ---
...
# Cache User 600s (10 phút) vì ít thay đổi
...
def login_user(u, p):
    ...
    if not row.empty and row.iloc[0]['password'] == make_hash(p): return row.iloc[0]
    return None
...
def create_user(u, p, n):
    ...
    try:
        ...
        st.cache_data.clear() # Xóa cache để cập nhật user mới
        return True
    except: return False

def update_user_role(u, r):
    sh = get_users_sheet()
    try: c = sh.find(u); sh.update_cell(c.row, 4, r); st.cache_data.clear()
    except: pass
...
# Cache Hồ sơ 60s (Giảm tải cực lớn cho Google)
...
# --- HÀM GHI (WRITE) - PHẢI XÓA CACHE SAU KHI GHI ---
def add_job(n, p, a, f, u, asn, d, is_survey, deposit_ok, fee_amount):
    ...
    sh.append_row([jid, now, n, p, a, "1. Tạo mới", "Đang xử lý", asn_clean, dl, link, log, 1 if is_survey else 0, 1 if deposit_ok else 0, fee_amount, 0])
    ...
    st.cache_data.clear() # Xóa cache để hiện ngay
...
def terminate_job(jid, rs, u):
    ...
    if cell:
        ...
        send_telegram_msg(f"⏹️ <b>KẾT THÚC</b>\n📂 <b>{code}</b>\n👤 Bởi: {u}\n📝 Lý do: {rs}")
        st.cache_data.clear()
...
# --- 6. UI MAIN ---
st.set_page_config(page_title="Đo Đạc Cloud V13.3", page_icon="☁️", layout="wide")

if 'logged_in' not in st.session_state: st.session_state['logged_in'] = False

if not st.session_state['logged_in']:
    st.title("🔐 Đăng nhập V13.3")
    c1, c2 = st.columns(2)
    with c1:
        ...
        if st.button("Login"):
            ...
            if d is not None: 
                ...
                st.session_state['user'] = d['username'] # Lấy theo tên cột
    ...
    if sel == "🏠 Việc Của Tôi":
        ...
        try:
            df = get_all_jobs_cached() # Dùng hàm cache
                    ...
                    for i, j in my_df.iterrows():
                        ...
                        with st.expander(f"{icon} {code} | {j['current_stage']}"):
                            ...
                            with tab_files:
                                ...
                                all_files = extract_all_files(j)
                                if not all_files: st.info("Chưa có file nào.")
                                else:
                            ...
                            with tab_info:
                                ...
                                with c1:
                                    ...
                                    with st.form(f"money_{j['id']}"):
                                        dep_ok = st.checkbox("Đã thu tạm ứng?", value=(safe_int(j.get('deposit')) == 1))
                                        if not dep_ok: st.caption("🔴 Chưa thu tạm ứng")
                                        fee = st.number_input("Phí đo đạc", value=safe_int(j.get('survey_fee')), step=100000)
                                        paid_ok = st.checkbox("Đã thu đủ tiền?", value=(safe_int(j.get('is_paid')) == 1))
                                        if st.form_submit_button("💾 Lưu Tài Chính"): update_finance_only(j['id'], dep_ok, fee, paid_ok, user); st.success("Lưu!"); time.sleep(0.5); st.rerun()
                                    st.markdown("#### 📜 Nhật ký")
                                    ...
                                    if j['status'] in ['Tạm dừng', 'Kết thúc sớm']:
                                        ...
                                        with st.form(f"f{j['id']}"):
                                            ...
                                            if st.form_submit_button("✅ Chuyển"): 
                                                ...
                                                update_stage(j['id'], cur, nt, fl, user, asn, d, is_sv, dep, money, pdone); st.success("Xong!"); time.sleep(0.5); st.rerun()
                                        ...
                                        if st.session_state.get(f'tm_{j['id']}', False):
                                            rst = st.text_input("Lý do kết thúc sớm:", key=f"rst{j['id']}")
                                            if st.button("OK Kết thúc", key=f"okt{j['id']}"): terminate_job(j['id'], rst, user); st.rerun()
        except Exception as e: st.error(f"Lỗi: {e}")

    # --- CÁC TAB KHÁC GIỮ NGUYÊN (Sử dụng hàm cache) ---
    elif sel == "💰 Công Nợ":
        st.title("💰 Quản Lý Công Nợ")
        try:
            df = get_all_jobs_cached()
            if not df.empty:
        ...
        with st.form("new"):
            ...
            c_m1, c_m2 = st.columns(2); dep_ok = c_m1.checkbox("Đã thu tạm ứng?"); fee_val = c_m2.number_input("Phí:", value=0, step=100000)
        ...
        if role == "Quản lý":
            st.title("Phân Quyền"); df = get_all_users_cached()
