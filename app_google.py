...
def update_stage(jid, stg, nt, f_list, u, asn, d, is_survey, deposit_ok, fee_amount, is_paid, result_date=None):
    ...
    if r:
        ...
        if nxt:
            ...
            if fee_amount is not None: sh.update_cell(r, 14, safe_int(fee_amount))
            if is_paid is not None: sh.update_cell(r, 15, 1 if is_paid else 0)
            
            olog = sh.cell(r, 11).value
            log_prefix = "⚡ BATCH:" if stg == "BATCH_UPDATE" else f"{stg}->{nxt}"
            note_content = nt if nt else ""
            nlog = f"\n[{now}] {u}: {log_prefix}{assign_str} | Note: {note_content}{log_file_str}"
            sh.update_cell(r, 11, olog + nlog)
            if nxt=="7. Hoàn thành": sh.update_cell(r, 7, "Hoàn thành")
            
            log_to_audit(u, "UPDATE_STAGE", f"ID: {jid}, {stg_display} -> {nxt}")
...
def render_job_card(j, user, role):
    ...
    with st.expander(f"{icon} {code_display} | {j['current_stage']}{stuck_alert}"):
        if status_bot == 2 and j['status'] == "Đang xử lý":
            st.error(f"⚠️ **CẢNH BÁO:** Hồ sơ đã nằm ở bước '{j['current_stage']}' quá {limit} giờ! (Đã {hours}h)")
        elif status_bot == 1 and j['status'] == "Đang xử lý":
            st.warning(f"⚠️ **LƯU Ý:** Hồ sơ sắp quá hạn xử lý bước '{j['current_stage']}' (Đã {hours}h/{limit}h)")
            ...
            if j['status'] in ['Tạm dừng', 'Kết thúc sớm']:
                ...
                with c_back.popover("⬅️ Trả hồ sơ", use_container_width=True):
                    ...
                    if st.button("Xác nhận", key=f"btn_back_{j['id']}"):
                        if return_to_previous_stage(j['id'], j['current_stage'], reason_back, user):
                            st.success("Đã trả hồ sơ!"); time.sleep(1); st.rerun()
                        else: st.error("Lỗi!")
...
if 'selected_batch_jobs' not in st.session_state: st.session_state['selected_batch_jobs'] = []
        ...
        with st.expander("⚡ Xử lý hàng loạt (Chọn hồ sơ bên dưới)", expanded=False):
            with st.form("batch_form"):
                ...
                if st.form_submit_button("🚀 Cập nhật tất cả hồ sơ đã chọn"):
                    if not st.session_state['selected_batch_jobs']:
                        st.error("Chưa chọn hồ sơ nào!")
                    else:
        ...
        if df.empty: 
            ...
            with c_note:
                if not warning_jobs.empty:
                    ...
                    if st.button("Xem DS Sắp Hết Hạn"): st.session_state['job_filter'] = 'warning_48h'
            ...
            if my_df.empty: st.info("Hết việc!")
                ...
                if k3.button(f"⛔ Tạm Dừng ({len(paused)})", use_container_width=True): st.session_state['job_filter'] = 'paused'
                if k4.button(f"🟢 Tổng ({len(my_df)})", use_container_width=True): st.session_state['job_filter'] = 'all'
                
                if st.session_state['job_filter'] == 'overdue': display_df = over
                elif st.session_state['job_filter'] == 'urgent': display_df = soon
                elif st.session_state['job_filter'] == 'paused': display_df = paused
                elif st.session_state['job_filter'] == 'warning_48h': display_df = warning_jobs
                ...
                for i, j in display_df.iterrows():
                    ...
                    with c_check:
                        ...
                        if is_checked:
                            if j['id'] not in st.session_state['selected_batch_jobs']:
                                st.session_state['selected_batch_jobs'].append(j['id'])
                        else:
