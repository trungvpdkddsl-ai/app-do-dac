...
def safe_int(value) -> int:
    try: 
        return int(float(str(value).replace(",", "").replace(".", ""))) if pd.notna(value) and value != "" else 0
    except: 
        return 0

def get_proc_abbr(proc_name: str) -> str:
    mapping = {
...
def extract_proc_from_log(log_text: str) -> str:
    match = re.search(r'Khởi tạo \((.*?)\)', str(log_text))
    return match.group(1) if match else ""

def get_next_stage_dynamic(current_stage: str, proc_name: str) -> str | None:
...
def check_bottleneck(deadline_str: str, current_stage: str) -> tuple[bool, int, int]:
    if current_stage == "8. Hoàn thành" or not deadline_str: 
        return False, 0, 0
    try:
...
def generate_unique_name(jid: int, start_time: str, name: str, phone: str, addr: str, proc_name: str) -> str:
    try:
...
def extract_files_from_log(log_text: str) -> list[tuple[str, str]]:
...
def format_precise_time(td: timedelta) -> str:
...
def get_processing_duration(logs: str, current_stage: str) -> tuple[timedelta, datetime | None]:
    if current_stage == "8. Hoàn thành" or not logs: 
...
def calculate_deadline(start_date: datetime, hours_to_add: int) -> datetime | None:
    if hours_to_add == 0: 
        return None
...
def get_drive_id(link: str) -> str | None:
    try: 
...
def get_status_badge_html(row: pd.Series) -> str:
...
# --- GOOGLE SHEETS & DRIVE API (Cải thiện cache) ---
def get_gcp_creds(): 
    return Credentials.from_service_account_info(st.secrets["gcp_service_account"], scopes=SCOPES)

@st.cache_data(ttl=300, hash_funcs={gspread.Worksheet: id})
def get_sheet_cached(sheet_name="DB_DODAC") -> gspread.Worksheet | None:
    try: 
        creds = get_gcp_creds(); 
...
def get_users_sheet() -> gspread.Worksheet | None:
    try:
        creds = get_gcp_creds(); 
        client = gspread.authorize(creds); 
...
def get_audit_sheet() -> gspread.Worksheet | None:
    try:
        creds = get_gcp_creds(); 
        client = gspread.authorize(creds); 
        sh = client.open("DB_DODAC")
        try: 
            return sh.worksheet("AUDIT_LOGS")
        except: 
...
def get_all_jobs_df_cached() -> pd.DataFrame:
    sh = get_sheet_cached()
    if sh is None: 
        return pd.DataFrame()
    ...
    if not df.empty:
        ...
        if 'start_time' in df.columns: 
            df['start_dt'] = pd.to_datetime(df['start_time'], errors='coerce').dt.date
    return df

def get_all_jobs_df() -> pd.DataFrame:
    return get_all_jobs_df_cached()

# --- FILE UPLOAD & ACTIONS ---
def upload_file_via_script(file_obj, sub_folder_name: str) -> tuple[str | None, str | None]:
    if not file_obj: 
        return None, None
    try:
        ...
        if response.status_code == 200:
            ...
            st.error(f"Lỗi mạng: {response.text}")
    except Exception as e: 
        st.error(f"Lỗi Upload: {e}")
    return None, None

def find_row_index(sh: gspread.Worksheet, jid: int) -> int | None:
    try: 
        ids = sh.col_values(1); 
...
def delete_file_system(job_id: int, file_link: str, file_name: str, user: str):
    ...
    if r:
        ...
        if sh.cell(r, 10).value == file_link: 
            sh.update_cell(r, 10, "")
...
def log_to_audit(user: str, action: str, details: str):
    def _log():
        try: 
            ws = get_audit_sheet(); 
            ws.append_row([datetime.now().strftime("%Y-%m-%d %H:%M:%S"), user, action, details])
        except: 
            pass
    threading.Thread(target=_log).start()

def login_user(u: str, p: str):
    sh = get_users_sheet(); 
    if not sh: 
        return None
    try: 
...
def create_user(u: str, p: str, n: str) -> bool:
    ...
    try: 
        ...
        sh.append_row([u, hashed_p, n, "Chưa cấp quyền"]); 
        get_all_users_cached.clear(); 
        return True
    except: 
        return False

def delete_user_permanently(u: str) -> bool:
    sh = get_users_sheet()
    try: 
        cell = sh.find(u); 
...
def get_all_users_cached() -> pd.DataFrame:
    sh = get_users_sheet()
...
def add_job(n: str, p: str, a: str, proc: str, f: list, u: str, asn: str):
    sh = get_sheet_cached(); 
    now = datetime.now(); 
...
def update_stage(jid: int, stg: str, nt: str, f_list: list, u: str, asn: str, 
                 d: int, is_survey: int, deposit_ok: bool, fee_amount: int, is_paid: bool, 
    ...
    # Tính next stage
    if nt in ["Đã nhận kết quả đúng hạn.", "Đã nhận kết quả sớm.", "Hoàn thành (Đã TT)"]:
        nxt = "8. Hoàn thành"
    else:
    ...
    if nxt:
        ...
        assign_tele = ""
        if asn: 
            assign_clean = asn.split(" - ")[0]; 
            sh.update_cell(r, 8, assign_clean)
            assign_str = f" -> Giao: {assign_clean}"; 
        ...
        nlog = f"\n[{now_str}] {u}: {stg}->{nxt}{assign_str} | Note: {nt}{log_file_str}"
...
def update_deadline_custom(jid: int, new_date: date, u: str):
    sh = get_sheet_cached(); 
    r = find_row_index(sh, jid)
    if r:
...
def return_to_previous_stage(jid: int, current_stage: str, reason: str, u: str) -> bool:
    sh = get_sheet_cached(); 
    r = find_row_index(sh, jid)
    if r:
        try:
...
def update_customer_info(jid: int, new_name: str, new_phone: str, new_addr: str, u: str):
    sh = get_sheet_cached(); 
    r = find_row_index(sh, jid)
    if r:
...
def update_finance_only(jid: int, deposit_ok: bool, fee_amount: int, is_paid: bool, u: str):
    sh = get_sheet_cached(); 
    r = find_row_index(sh, jid)
    if r:
        row_data = sh.row_values(r)
...
def pause_job(jid: int, rs: str, u: str):
    sh = get_sheet_cached(); 
    r = find_row_index(sh, jid)
    if r:
        row_data = sh.row_values(r)
...
def terminate_job(jid: int, rs: str, u: str):
    sh = get_sheet_cached(); 
    r = find_row_index(sh, jid)
    if r:
        row_data = sh.row_values(r)
...
def move_to_trash(jid: int, u: str):
    sh = get_sheet_cached(); 
    r = find_row_index(sh, jid)
    if r: 
...
def restore_from_trash(jid: int, u: str):
    sh = get_sheet_cached(); 
    r = find_row_index(sh, jid)
    if r: 
        sh.update_cell(r, 7, "Đang xử lý"); 
...
def delete_forever(jid: int, u: str):
    sh = get_sheet_cached(); 
    r = find_row_index(sh, jid)
    if r: 
        sh.delete_rows(r); 
...
# --- UI COMPONENTS ---
def change_menu(new_menu: str):
    st.session_state['menu_selection'] = new_menu

def render_square_menu(role: str):
...
def render_job_card_content(j: pd.Series, user: str, role: str, user_list: list[str]):
    try: 
        dl_dt = pd.to_datetime(j['deadline'])
    except: 
        dl_dt = datetime.now() + timedelta(days=365)
            ...
            with st.container():
                ...
                for idx, (fname, link) in enumerate(file_list):
                    ...
                    with c_ico: 
                        st.write("📄")
                    with c_name: 
                        st.markdown(f"<span style='font-size:13px; position:relative; top:2px'>{fname}</span>", unsafe_allow_html=True)
                    with c_view: 
                        st.link_button("👁️", link, help="Xem file") 
                    with c_down: 
                        st.link_button("⬇️", down_link, help="Tải về")
                    with c_del:
                        if role == "Quản lý":
    ...
    with t2:
        if j['status'] in ['Tạm dừng', 'Kết thúc sớm']:
            ...
            if j['status'] == 'Tạm dừng' and st.button("▶️ Tiếp tục", key=f"r{j['id']}"): 
            ...
            st.divider()
            c1, c2 = st.columns(2)
            if c1.button("⏸️ Dừng", key=f"p{j['id']}", use_container_width=True): 
            ...
            with c2.popover("⬅️ Trả hồ sơ", use_container_width=True):
                ...
                if st.button("Xác nhận", key=f"cb_{j['id']}"): 
                    return_to_previous_stage(j['id'], j['current_stage'], reason, user); 
                    st.rerun()
            ...
            with st.form(f"f{j['id']}"):
                ...
                with c_assign:
                    if nxt != "8. Hoàn thành":
                        idx = 0
                        if user_list and j['assigned_to'] in user_list: 
                            idx = user_list.index(j['assigned_to'])
            ...
            if c_pause.button("⏸️", key=f"p{j['id']}", help="Tạm dừng"): 
                st.session_state[f'pm_{j['id']}'] = True
            if c_term.button("⏹️", key=f"t{j['id']}", help="Kết thúc sớm"): 
                st.session_state[f'tm_{j['id']}'] = True
            with c_back.popover("⬅️", help="Trả hồ sơ"):
                reason = st.text_input("Lý do:", key=f"rb_{j['id']}")
                if st.button("Trả về", key=f"cb_{j['id']}"): 
                    return_to_previous_stage(j['id'], j['current_stage'], reason, user); 
                    st.rerun()

        if st.session_state.get(f'pm_{j['id']}', False):
            rs = st.text_input("Lý do dừng:", key=f"rs{j['id']}")
            if st.button("Xác nhận dừng", key=f"okp{j['id']}"): 
        ...
        if st.session_state.get(f'tm_{j['id']}', False):
            ...
            if st.button("Xác nhận kết thúc", key=f"okt{j['id']}"): 
                terminate_job(j['id'], rst, user); 
                st.rerun()
...
def render_complex_list_view(df: pd.DataFrame, user: str, role: str, user_list: list[str]):
    ...
    pag_df = df.iloc[start_idx:end_idx]
    
    for index, row in pag_df.iterrows():
        short_id = str(row['id'])
        if len(short_id) > 6: 
        ...
        if not proc_name: 
            proc_name = "Đo đạc khác"
        
        start_dt = pd.to_datetime(row['start_time'])
        try: 
            dl_dt = pd.to_datetime(row['deadline'])
        except: 
            dl_dt = None
        ...
        with st.container(border=True):
            ...
            with c2: 
                st.markdown(f"<div class='proc-name'>{proc_name}</div>", unsafe_allow_html=True)
            ...
            with c6: 
                st.markdown(get_status_badge_html(row), unsafe_allow_html=True)
    ...
    if total_pages > 1:
        ...
        if col1.button("◀️ Trước"): 
            st.session_state['page'] = max(1, page - 1); 
            st.rerun()
        col2.write(f"Trang {page}/{total_pages}")
        if col3.button("Sau ▶️"): 
...
if 'menu_selection' not in st.session_state: 
    st.session_state['menu_selection'] = "🏠 Việc Của Tôi"
        ...
        if not df.empty:
            ...
            if not urgent.empty:
                ...
                if role == "Quản lý":
                    ...
                    for u_name, c in counts.items(): 
                        st.caption(f"- {u_name}: {c}")
                else:
                    my_urgent = urgent[urgent['assigned_to'].str.contains(user, na=False)]
                    if not my_urgent.empty: 
                        st.error(f"Bạn có {len(my_urgent)} hồ sơ gấp!")
        st.markdown("---"); 
        render_square_menu(role); 
        st.markdown("---")
    ...
    if sel == "🏠 Việc Của Tôi":
        ...
        if df.empty: 
            ...
            if my_df.empty: 
                ...
                if k1.button(f"🔴 Quá Hạn ({count_overdue})", use_container_width=True): 
                    st.session_state['job_filter'] = 'overdue'
                if k2.button(f"🟡 Sắp đến hạn ({count_soon})", use_container_width=True): 
                    st.session_state['job_filter'] = 'urgent'
                if k3.button(f"⛔ Tạm dừng ({count_paused})", use_container_width=True): 
                    st.session_state['job_filter'] = 'paused'
                if k4.button(f"🟢 Tổng ({count_total})", use_container_width=True): 
                    st.session_state['job_filter'] = 'all'
                ...
                if st.session_state['job_filter'] == 'overdue': 
                    display_df = my_df[(my_df['dl_dt'] < now) & (my_df['status'] != 'Tạm dừng')]
                elif st.session_state['job_filter'] == 'urgent': 
                    display_df = my_df[(my_df['dl_dt'] >= now) & (my_df['dl_dt'] <= now + timedelta(hours=24)) & (my_df['status'] != 'Tạm dừng')]
                elif st.session_state['job_filter'] == 'paused': 
                    display_df = my_df[my_df['status'] == 'Tạm dừng']
                else: 
                    display_df = my_df
        ...
        if df.empty: 
            ...
            if archive_df.empty: 
                st.info("Chưa có hồ sơ nào đã hoàn thành.")
            else:
                ...
                if not unique_years: 
                    unique_years = [datetime.now().year]
        ...
        with c3: 
            is_sv = st.checkbox("🛠️ CHỈ ĐO ĐẠC")
        with c4: 
        ...
        if st.button("Tạo Hồ Sơ", type="primary"):
            if n and asn: 
                ...
                st.error("Thiếu tên hoặc người giao việc!")
                    ...
                    with c3:
                        if u['username']!=user:
                            if st.button("🗑️", key=f"del_u_{u['username']}"): 
                                delete_user_permanently(u['username']); 
                                st.rerun()
        else: 
            st.error("Cấm truy cập!")

    elif sel == "🗑️ Thùng Rác":
        if role == "Quản lý":
            st.title("🗑️ Thùng Rác"); 
        ...
        if role == "Quản lý":
            ...
            if audit_sheet: 
                st.dataframe(pd.DataFrame(audit_sheet.get_all_records()), use_container_width=True)
        else: 
            st.error("Cấm truy cập!")
