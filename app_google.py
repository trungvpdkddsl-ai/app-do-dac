...
def add_job(n, p, a, f, u, asn, d, is_survey, deposit_ok, fee_amount):
    ...
    money_msg = "✅ Đã tạm ứng" if deposit_ok else "❌ Chưa tạm ứng"
...
def update_finance_only(jid, deposit_ok, fee_amount, is_paid, u):
    ...
    if cell:
        ...
        c_name = sh.cell(r, 3).value; start_t = sh.cell(r, 2).value
        code = generate_code(jid, start_t, c_name)
        send_telegram_msg(f"💰 <b>TÀI CHÍNH</b>\n📂 <b>{code}</b>\n👤 {u}\nPhí: {fee_amount:,} VNĐ | Đã thu: {'✅' if is_paid else '❌'}")
...
# --- 6. UI MAIN ---
st.set_page_config(page_title="Đo Đạc V10", page_icon="💰", layout="wide")

if 'logged_in' not in st.session_state: st.session_state['logged_in'] = False

if not st.session_state['logged_in']:
    st.title("🔐 Đăng nhập V10.0")
    c1, c2 = st.columns(2)
    with c1:
    ...
    if role == "Quản lý": 
        menu.insert(1, "💰 Công Nợ") # Chèn Công Nợ vào Menu chính
        menu.append("👥 Nhân Sự")
    ...
    if sel == "🏠 Việc Của Tôi":
        ...
        try:
            ...
            if df.empty: st.info("Trống!")
                ...
                my_df = df if role == "Quản lý" else df[(df['assigned_to'].astype(str) == user) & (~df['status'].isin(['Hoàn thành', 'Kết thúc sớm']))]
                        ...
                        with st.expander(f"{icon} {code} | {j['current_stage']}"):
                            ...
                            with c1:
                                ...
                                with st.form(f"money_{j['id']}"):
                                    ...
                                    dep_ok = st.checkbox("Đã thu tạm ứng?", value=dep_val); fee = st.number_input("Chi phí đo đạc (VNĐ)", value=fee_val, step=100000); paid_ok = st.checkbox("Đã thu đủ tiền?", value=paid_val)
