...
def extract_all_files(row):
    """Tìm tất cả file từ Cả Cột File Gốc và Cột Logs"""
    files = []
    # 1. Lấy file gốc (Cột file_link)
    if row.get('file_link') and str(row.get('file_link')).startswith('http'):
        files.append(row['file_link'])
    
    # 2. Lấy file trong Logs
    log_text = str(row.get('logs', ''))
    log_links = re.findall(r'(https?://[^\s]+)', log_text)
    for link in log_links:
        if link not in files: # Tránh trùng
            files.append(link)
...
def upload_to_drive(file_obj, folder_name):
    ...
    try:
        ...
        if res_sub.get('files'): fid = res_sub['files'][0]['id']
        else: fid = service.files().create(body={'name': folder_name, 'mimeType': 'application/vnd.google-apps.folder', 'parents': [pid]}, fields='id').execute().get('id')
...
def update_user_role(u, r):
    sh = get_users_sheet(); 
    try: c = sh.find(u); sh.update_cell(c.row, 4, r)
    except: pass
...
def add_job(n, p, a, f, u, asn, d, is_survey, deposit_ok, fee_amount):
    ...
    if link: log += f" | File: {link}" # Ghi link vào log luôn
...
def update_stage(jid, stg, nt, f, u, asn, d, is_survey, deposit_ok, fee_amount, is_paid):
    ...
    if cell:
        ...
        if nxt:
            ...
            if lnk: new_log_entry += f" | File: {lnk}"
            
            sh.update_cell(r, 11, olog + new_log_entry)
...
# --- 6. UI MAIN ---
st.set_page_config(page_title="Đo Đạc Cloud V13", page_icon="☁️", layout="wide")

if 'logged_in' not in st.session_state: st.session_state['logged_in'] = False

if not st.session_state['logged_in']:
    st.title("🔐 Đăng nhập V13.0")
    c1, c2 = st.columns(2)
    with c1:
    ...
    with c2:
        ...
        if st.button("Đăng Ký"):
            if create_user(nu, np, nn): st.success("OK! Chờ duyệt."); else: st.error("Trùng tên!")
else:
                ...
                if my_df.empty: st.info("Hết việc!")
                    ...
                    for i, j in my_df.iterrows():
                        ...
                        with st.expander(f"{icon} {code} | {j['current_stage']}"):
                            ...
                            # --- TÁCH TAB FILE ---
                            tab_info, tab_files = st.tabs(["📝 Xử lý & Tài chính", "📂 Hồ sơ đính kèm"])
                            
                            with tab_files:
                                st.markdown("### 📂 Danh sách file của hồ sơ này")
                                all_files = extract_all_files(j) # Hàm mới tự tìm hết file
                                if not all_files:
    ...
    # --- CÁC TAB KHÁC GIỮ NGUYÊN ---
        ...
        try:
            ...
            if not df.empty:
                ...
                st.metric("Hồ sơ chưa thu tiền", len(unpaid))
                if not unpaid.empty:
