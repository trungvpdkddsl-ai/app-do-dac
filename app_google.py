...
def add_job(n, p, a, f, u, asn, d, is_survey, deposit, fee):
    ...
    c.execute("INSERT INTO jobs (customer_name, customer_phone, address, current_stage, status, assigned_to, start_time, deadline, last_updated, is_survey_only, deposit, survey_fee, is_paid) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,0)", (n, p, a, "1. Tạo mới", "Đang xử lý", asn_c, now, dl, now, 1 if is_survey else 0, 1 if deposit else 0, fee))
...
def update_stage(jid, stg, nt, f, u, asn, d, is_survey, deposit, fee, is_paid, customer_name):
    conn = get_connection()
    ...
    if nxt:
        ...
        save_log_entry(jid, stg, u, f"{nt} (Chuyển: {asn_c})", fp)
                        ...
                        if j['status'] in ['Tạm dừng', 'Kết thúc sớm']:
                            ...
                            with st.form(f"f{j['id']}"):
                                ...
                                if st.form_submit_button("✅ Chuyển bước"): 
                                    ...
                                    update_stage(j['id'], cur, nt, fl, user, asn, d, j.get('is_survey_only'), dep, fee, pdone, j['customer_name']); st.success("Xong!"); time.sleep(0.5); st.rerun()
