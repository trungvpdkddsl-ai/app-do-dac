...
def add_job(n, p, a, f, u, asn, d, is_survey):
    ...
    # Tạo mã đẹp để gửi tele
    code = generate_code(jid, now, n, p, a)
    type_msg = "(CHỈ ĐO ĐẠC)" if is_survey else ""
    
    msg = f"🚀 <b>TIẾP NHẬN MỚI {type_msg}</b>\n📂 <b>{code}</b>\n📍 {a}\n👉 Giao cho: <b>{asn_c}</b>\n⏰ Hạn: {d} ngày"
    send_telegram_msg(msg)
    save_log(jid, "1. Tạo mới", u, f"Khởi tạo {type_msg} (Hạn {d} ngày)", f)

def update_stage(jid, stg, nt, f, u, asn, d):
    conn = get_connection(); c = conn.cursor()
    # Lấy thông tin hồ sơ để tạo nội dung tin nhắn
    c.execute("SELECT start_time, customer_name, customer_phone, address, is_survey_only FROM jobs WHERE id=?", (jid,))
    job_info = c.fetchone()
    
    is_survey = job_info[4] if job_info else 0
    ...
    if nxt:
        ...
        c.execute("UPDATE jobs SET current_stage=?, status=?, assigned_to=?, deadline=?, last_updated=? WHERE id=?", (nxt, "Hoàn thành" if nxt=="7. Hoàn thành" else "Đang xử lý", asn_c, dl, now, jid))
        conn.commit()
        
        # Gửi Telegram đẹp
        if job_info:
            code = generate_code(jid, job_info[0], job_info[1], job_info[2], job_info[3])
...
def pause_job(jid, rs, u):
    conn = get_connection(); c = conn.cursor()
    c.execute("UPDATE jobs SET status='Tạm dừng' WHERE id=?", (jid,))
    conn.commit()
    
    # Lấy info để báo tin
    c.execute("SELECT start_time, customer_name, customer_phone, address FROM jobs WHERE id=?", (jid,))
    row = c.fetchone()
    conn.close()
    
    if row:
        code = generate_code(jid, row[0], row[1], row[2], row[3])
...
def resume_job(jid, u):
    conn = get_connection(); c = conn.cursor()
    c.execute("UPDATE jobs SET status='Đang xử lý' WHERE id=?", (jid,))
    conn.commit()
...
def search_jobs(q, s_date=None, e_date=None):
    ...
    # Tạo cột mã full để hiển thị và tìm kiếm
