...
# Quy trình mặc định (sẽ được code xử lý động)
WORKFLOW_DEFAULT = {
    "1. Tạo mới": "2. Đo đạc", 
...
def get_all_jobs_df():
    ...
    if not df.empty:
        ...
        if 'procedure_type' not in df.columns: df['procedure_type'] = "Cấp đổi" # Mặc định
    return df
...
def add_job(n, p, a, f, u, asn, d, is_survey, deposit_ok, fee_amount, proc_type):
    ...
    # Logic chuyển bước đầu tiên dựa vào Thủ tục
    current_stg = "1. Tạo mới"
    
    # Thêm vào Sheet (16 cột: thêm procedure_type)
    sh.append_row([jid, now, n, p, a, current_stg, "Đang xử lý", asn_clean, dl, link, log, sv_flag, dep_flag, fee_amount, 0, proc_type])
    ...
    # Nếu chuyển quyền -> Bỏ qua đo đạc -> Bước tiếp theo sẽ là Làm hồ sơ
    next_step_hint = "Làm hồ sơ" if proc_type == "Chuyển quyền" else "Đo đạc"
    
    send_telegram_msg(f"🚀 <b>MỚI {type_msg}</b>\n📂 <b>{code}</b>\n📍 {a}\n👉 Giao {next_step_hint}: {asn_clean}\n💰 {money_msg}")
...
def update_stage(jid, stg, nt, f, u, asn, d, is_survey, deposit_ok, fee_amount, is_paid, proc_type):
    ...
    if cell:
        ...
        # Trường hợp 1: Hồ sơ Chuyển quyền (Bỏ qua Đo đạc)
        if proc_type == "Chuyển quyền" and stg == "1. Tạo mới":
            nxt = "3. Làm hồ sơ"
        
        # Trường hợp 2: Hồ sơ Chỉ đo đạc (Nhảy cóc về đích)
...
# --- 6. UI MAIN ---
st.set_page_config(page_title="Đo Đạc Cloud V14", page_icon="☁️", layout="wide")

if 'logged_in' not in st.session_state: st.session_state['logged_in'] = False

if not st.session_state['logged_in']:
    st.title("🔐 Đăng nhập V14.0")
    c1, c2 = st.columns(2)
    with c1:
                    ...
                    for i, j in my_df.iterrows():
                        ...
                        with st.expander(f"{icon} {code} | {j['current_stage']}"):
                            ...
                            with tab_files:
                                ...
                                if not unique_links: st.info("Chưa có file nào.")
                                else:
                                    for link in unique_links:
                                        c_f1, c_f2 = st.columns([3, 1])
                                        c_f1.markdown(f"🔗 [Mở Link]({link})"); c_f2.link_button("⬇️ Xem/Tải", link)
                            ...
                            with tab_info:
                                ...
                                with c1:
                                    ...
                                    with st.form(f"money_{j['id']}"):
                                        ...
                                        if st.form_submit_button("💾 Lưu Tài Chính"): update_finance_only(j['id'], dep_ok, fee, paid_ok, user); st.success("Đã lưu!"); time.sleep(0.5); st.rerun()
                                    st.markdown("#### 📜 Nhật ký")
                                ...
                                with c2:
                                    if j['status'] in ['Tạm dừng', 'Kết thúc sớm']:
                                        ...
                                        with st.form(f"f{j['id']}"):
                                            ...
                                            # LOGIC QUY TRÌNH THÔNG MINH
                                            proc = j.get('procedure_type', 'Cấp đổi')
                                            
                                            # 1. Chuyển quyền: Bỏ qua Đo đạc
                                            if proc == "Chuyển quyền" and cur == "1. Tạo mới":
                                                nxt = "3. Làm hồ sơ"
                                            # 2. Chỉ đo đạc: Nhảy cóc về đích
    ...
    # --- CÁC TAB KHÁC GIỮ NGUYÊN ---
        ...
        with st.form("new"):
            ...
            d = st.number_input("Hạn xử lý (Ngày)", value=1)
            
            if st.form_submit_button("Tạo"):
                if n and asn:
                    # Nếu chọn chuyển quyền thì tick "is_survey" không có ý nghĩa lắm, nhưng cứ truyền vào
