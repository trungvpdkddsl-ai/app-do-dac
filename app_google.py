...
def generate_excel_download(df):
    ...
    # Làm sạch người thực hiện
    export_df['assigned_to'] = export_df['assigned_to'].apply(lambda x: x.split(' - ')[0] if x else "Chưa giao")
    
    # Chọn cột và sắp xếp khoa học: [Mã, Thủ tục] -> [Ai làm gì] -> [Khách] -> [Tài chính]
    final_df = export_df[[
    ...
    with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
        ...
        # Chỉnh độ rộng cột
        worksheet.set_column('A:A', 15) # Mã
        worksheet.set_column('B:B', 20) # Thủ tục
        worksheet.set_column('C:C', 25) # Bước
        worksheet.set_column('D:D', 20) # Người làm
        worksheet.set_column('E:E', 15) # Trạng thái
        worksheet.set_column('F:F', 30) # Tên khách
