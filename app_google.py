...
def generate_code(jid, start, name):
    try:
        d = pd.to_datetime(start).strftime('%d%m%y')
    except:
...
def init_db():
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS jobs (
        ...
        current_stage TEXT, status TEXT, assigned_to TEXT, 
        start_time TIMESTAMP, deadline TIMESTAMP, last_updated TIMESTAMP, 
        is_survey_only INTEGER DEFAULT 0, 
        deposit INTEGER DEFAULT 0, 
        survey_fee INTEGER DEFAULT 0, 
        is_paid INTEGER DEFAULT 0
    )''')
    c.execute('''CREATE TABLE IF NOT EXISTS logs (
        id INTEGER PRIMARY KEY AUTOINCREMENT, 
        job_id INTEGER, stage TEXT, action_by TEXT, 
        note TEXT, file_path TEXT, timestamp TIMESTAMP
    )''')
    
    # Migration (Thêm cột nếu thiếu)
    try: c.execute("ALTER TABLE jobs ADD COLUMN is_survey_only INTEGER DEFAULT 0")
    except: pass
    try: c.execute("ALTER TABLE jobs ADD COLUMN deposit INTEGER DEFAULT 0")
    except: pass
    try: c.execute("ALTER TABLE jobs ADD COLUMN survey_fee INTEGER DEFAULT 0")
    except: pass
    try: c.execute("ALTER TABLE jobs ADD COLUMN is_paid INTEGER DEFAULT 0")
    except: pass
...
def create_user(u, p, n, r="Chưa cấp quyền"):
    conn = get_connection()
    try:
        conn.execute("INSERT INTO users VALUES (?, ?, ?, ?)", (u, make_hash(p), n, r))
        conn.commit()
        return True
    except:
        return False
...
def login_user(u, p):
    conn = get_connection()
    d = conn.execute("SELECT * FROM users WHERE username=? AND password=?", (u, make_hash(p))).fetchall()
    conn.close()
    return d

def get_active_users():
    conn = get_connection()
    d = conn.execute("SELECT username, fullname FROM users WHERE role!='Chưa cấp quyền'").fetchall()
    conn.close()
    return [f"{u[0]} - {u[1]}" for u in d]

def get_all_users():
    conn = get_connection()
    df = pd.read_sql_query("SELECT username, fullname, role FROM users", conn)
    conn.close()
    return df
...
def add_job(n, p, a, f, u, asn_list, d, is_survey, deposit, fee):
    ...
    c.execute("INSERT INTO jobs (customer_name, customer_phone, address, current_stage, status, assigned_to, start_time, deadline, last_updated, is_survey_only, deposit, survey_fee, is_paid) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,0)", 
              (n, p, a, "1. Tạo mới", "Đang xử lý", asn_str, now, dl, now, 1 if is_survey else 0, 1 if deposit else 0, fee))
    jid = c.lastrowid
    conn.commit()
    conn.close()
...
def update_stage(jid, stg, nt, f, u, asn_list, d, is_survey, deposit, fee, is_paid, customer_name):
    ...
    if nxt:
        ...
        # Lấy lại thông tin để tạo mã
        conn2 = get_connection()
        c2 = conn2.cursor()
        c2.execute("SELECT start_time FROM jobs WHERE id=?", (jid,))
        row = c2.fetchone()
        conn2.close()
...
def pause_job(jid, rs, u):
    conn = get_connection()
    conn.execute("UPDATE jobs SET status='Tạm dừng' WHERE id=?", (jid,))
    conn.commit()
    conn.close()
...
def resume_job(jid, u):
    conn = get_connection()
    conn.execute("UPDATE jobs SET status='Đang xử lý' WHERE id=?", (jid,))
    conn.commit()
    conn.close()
...
def terminate_job(jid, rs, u):
    conn = get_connection()
    conn.execute("UPDATE jobs SET status='Kết thúc sớm' WHERE id=?", (jid,))
    conn.commit()
    conn.close()
...
def get_my_jobs(u, r):
    ...
    # Nếu là quản lý thì thấy hết, nhân viên thì lọc
    if r == "Quản lý":
        return df
    else:
...
def get_logs(jid):
    conn = get_connection()
    return pd.read_sql_query("SELECT * FROM logs WHERE job_id=? ORDER BY timestamp DESC", conn, params=(jid,))
...
if not st.session_state['logged_in']:
    ...
    with c1:
        ...
        if st.button("Login"):
            ...
            if d: 
                ...
                st.session_state['user']=d[0][0]
                st.session_state['role']=d[0][3]
                st.rerun()
            else: 
    ...
    if not df_all.empty:
        ...
        # Lọc thông báo cho user
        def check_notify(assigned_str):
             if not assigned_str: return False
             return user in str(assigned_str).split(", ")
        
        if role != "Quản lý":
            my_alert_df = df_all[df_all['assigned_to'].apply(check_notify) & (~df_all['status'].isin(['Hoàn thành', 'Kết thúc sớm']))]
        else:
            my_alert_df = df_all[~df_all['status'].isin(['Hoàn thành', 'Kết thúc sớm'])]
