...
import requests
import threading
import calendar # Thư viện lịch cho bộ lọc tháng
...
# --- 2. HÀM VISUAL (Thanh tiến độ đẹp) ---
def render_progress_bar(current_stage, status):
    ...
    <style>
        ...
        .done {{background: #28a745;}} .active {{background: {color}; color: black; box-shadow: 0 0 0 3px rgba(0,0,0,0.1);}} .pending {{background: #e9ecef; color: #999;}}
...
def init_db():
    conn = get_connection()
    conn.execute('''CREATE TABLE IF NOT EXISTS users (username TEXT PRIMARY KEY, password TEXT, fullname TEXT, role TEXT)''')
    conn.execute('''CREATE TABLE IF NOT EXISTS jobs (id INTEGER PRIMARY KEY AUTOINCREMENT, customer_name TEXT, customer_phone TEXT, address TEXT, current_stage TEXT, status TEXT, assigned_to TEXT, start_time TIMESTAMP, deadline TIMESTAMP, last_updated TIMESTAMP)''')
    conn.execute('''CREATE TABLE IF NOT EXISTS logs (id INTEGER PRIMARY KEY AUTOINCREMENT, job_id INTEGER, stage TEXT, action_by TEXT, note TEXT, file_path TEXT, timestamp TIMESTAMP)''')
...
def add_job(n, p, a, f, u, asn, d):
    ...
    c.execute("INSERT INTO jobs (customer_name, customer_phone, address, current_stage, status, assigned_to, start_time, deadline, last_updated) VALUES (?,?,?,?,?,?,?,?,?)", (n, p, a, "1. Tạo mới", "Đang xử lý", asn_c, now, dl, now))
    jid = c.lastrowid; conn.commit(); conn.close()
    send_telegram_msg(f"🚀 <b>MỚI #{jid}</b>\n👤 {n}\n👉 Giao: {asn_c}")
    save_log(jid, "1. Tạo mới", u, f"Khởi tạo (Hạn {d} ngày)", f)

def update_stage(jid, stg, nt, f, u, asn, d):
    conn = get_connection(); nxt = WORKFLOW_DEFAULT.get(stg); asn_c = asn.split(" - ")[0] if asn else None
    if nxt:
...
def search_jobs(q):
    conn = get_connection(); df = pd.read_sql_query("SELECT * FROM jobs ORDER BY id DESC", conn); conn.close()
...
if not st.session_state['logged_in']:
    ...
    if sel == "🏠 Việc Của Tôi":
        ...
        if df.empty: st.info("Hết việc!")
            ...
            c1.metric("🔴 Quá hạn", len(over)); c2.metric("🟡 Gấp", len(soon)); c3.metric("🟢 Tổng", len(df))
            st.divider()
                    ...
                    with c2:
                        if j['status']=='Tạm dừng':
                            ...
                            with st.form(f"f{j['id']}"):
                                nt = st.text_area("Ghi chú"); fl = st.file_uploader("KQ")
                                cur = j['current_stage']; nxt = WORKFLOW_DEFAULT.get(cur); asn = None; d = 1
        ...
        if q:
            df = search_jobs(q); st.write(f"Kết quả: {len(df)}")
            for i, j in df.iterrows():
                with st.expander(f"{j['full']} ({j['status']})"):
                    render_progress_bar(j['current_stage'], j['status']); st.subheader(f"👤 {j['customer_name']}")
                    st.write(f"Người làm: {j['assigned_to']}"); st.markdown("**Lịch sử:**")
                    logs = get_logs(j['id'])
                    for x, l in logs.iterrows():
        ...
        with st.form("new"):
            ...
            if st.form_submit_button("Tạo"):
                if n and asn: add_job(n, p, a, f, user, asn, d); st.success("OK!"); time.sleep(1)
                else: st.error("Thiếu tin!")

    elif sel == "📊 Báo Cáo":
        st.title("Thống Kê")
        c1, c2 = st.columns(2)
        ft = c1.radio("Xem theo:", ["Tháng", "Ngày"], horizontal=True)
        if ft == "Tháng":
            m = c2.selectbox("Tháng", range(1, 13), index=datetime.now().month-1)
            y = c2.number_input("Năm", value=datetime.now().year)
            s = datetime(y, m, 1); e = datetime(y, m, calendar.monthrange(y, m)[1])
        else:
            s = c2.date_input("Từ", datetime.now()-timedelta(30)); e = c2.date_input("Đến", datetime.now())
        ...
        if not df.empty:
            ...
            k1.metric("Tổng", tot, border=True); k2.metric("Đang làm", proc, border=True)
            k3.metric("Xong", fin, border=True); k4.metric("Trễ", late, border=True, delta_color="inverse")
            
            st.divider()
            c_ch1, c_ch2 = st.columns([2, 1])
            sc = df['current_stage'].value_counts().reset_index(); sc.columns=['Giai đoạn', 'SL']
            c_ch1.bar_chart(sc, x='Giai đoạn', y='SL')
            
            dt = pd.DataFrame({'SL': [proc-late, late, fin]}, index=['Đúng hạn', 'Trễ', 'Xong'])
            dt['%'] = (dt['SL']/tot*100).round(1)
            ...
            st.dataframe(df[['Mã', 'customer_name', 'current_stage', 'assigned_to', 'TT']], use_container_width=True)
